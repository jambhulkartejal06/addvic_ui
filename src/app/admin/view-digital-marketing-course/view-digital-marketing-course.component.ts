...
@Component({
  selector: 'app-view-digital-marketing-course',
  templateUrl: './view-digital-marketing-course.component.html',
  styleUrls: ['./view-digital-marketing-course.component.css']
})
export class ViewDigitalMarketingCourseComponent implements OnInit {

  ELEMENT_DATA : CourseDigitalMarketing[];
  displayedColumns: string [] = ['course_id','courseName','basicPlan','premiumPlan','categories','action'];
  
  dataSource = new MatTableDataSource<CourseDigitalMarketing>();
  ...
  public getAllCourses(){
    let res=this.courseservice.getAllCreateDigitalMarktingCourse();
    res.subscribe(courses=>this.dataSource.data = courses as CourseDigitalMarketing[]);
  }
  ...
  onDelete(course,index){
    ...
    removeData(course,index){
      ...
      this.courseservice.deleteDigitalMarktingCourses(course.course_id).subscribe(res => {
      });
        this.toastr.success('Data Deleted Successfully !!!!!', 'Toastr fun!', {
          timeOut: 1000
